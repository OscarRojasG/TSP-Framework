...
def create_data_model(
    distance_matrix, scale_factor=10000, start_node=None, end_node=None
):
    ...
    data["scaled_distance_matrix"] = [
        [int(dist * scale_factor) for dist in row] for row in data["distance_matrix"]
    ]
    data["num_vehicles"] = 1
...
def solve(instance: TSPInstance, start_node=None, end_node=None, time_limit=0):
    data = create_data_model(
        instance.distance_matrix, start_node=start_node, end_node=end_node
    )
